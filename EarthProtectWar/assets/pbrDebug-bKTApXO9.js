import{b5 as e}from"./index-CZGb7Fdp.js";import"./logDepthDeclaration-B8Xd6jHP.js";import"./oitFragment-DAwSurGf.js";const t="pbrFragmentExtraDeclaration",g=`varying vec3 vPositionW;
#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[t]||(e.IncludesShadersStore[t]=g);const M={name:t,shader:g},o="subSurfaceScatteringFunctions",u=`bool testLightingForSSS(float diffusionProfile)
{return diffusionProfile<1.;}`;e.IncludesShadersStore[o]||(e.IncludesShadersStore[o]=u);const F={name:o,shader:u},i="importanceSampling",E=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;e.IncludesShadersStore[i]||(e.IncludesShadersStore[i]=E);const O={name:i,shader:E},a="pbrHelperFunctions",h=`#define MINIMUMVARIANCE 0.0005
...
#endif
`;e.IncludesShadersStore[a]||(e.IncludesShadersStore[a]=h);const I={name:a,shader:h},r="pbrDirectLightingSetupFunctions",L=`struct preLightingInfo
...
result.areaLightDiffuse=data.Diffuse;result.LdotV=0.;result.roughness=0.;result.diffuseRoughness=0.;result.surfaceAlbedo=vec3(0.);return result;}
#endif
`;e.IncludesShadersStore[r]||(e.IncludesShadersStore[r]=L);const _={name:r,shader:L},n="pbrDirectLightingFalloffFunctions",p=`float computeDistanceLightFalloff_Standard(vec3 lightOffset,float range)
...
#endif
}`;e.IncludesShadersStore[n]||(e.IncludesShadersStore[n]=p);const R={name:n,shader:p},l="hdrFilteringFunctions",D=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;e.IncludesShadersStore[l]||(e.IncludesShadersStore[l]=D);const B={name:l,shader:D},f="pbrDirectLightingFunctions",m=`#define CLEARCOATREFLECTANCE90 1.0
struct lightingInfo
{vec3 diffuse;
...
#endif
`;e.IncludesShadersStore[f]||(e.IncludesShadersStore[f]=m);const b={name:f,shader:m},s="pbrBlockNormalGeometric",v=`vec3 viewDirectionW=normalize(vEyePosition.xyz-vPositionW);
#ifdef NORMAL
...
#endif
`;e.IncludesShadersStore[s]||(e.IncludesShadersStore[s]=v);const U={name:s,shader:v},c="pbrBlockImageProcessing",A=`#if defined(IMAGEPROCESSINGPOSTPROCESS) || defined(SS_SCATTERING)
...
#endif
`;e.IncludesShadersStore[c]||(e.IncludesShadersStore[c]=A);const G={name:c,shader:A},d="pbrDebug",C=`#if DEBUGMODE>0
...
#endif
`;e.IncludesShadersStore[d]||(e.IncludesShadersStore[d]=C);const P={name:d,shader:C};export{U as a,P as b,R as c,b as d,_ as e,M as f,I as g,B as h,O as i,G as p,F as s};
